...
import numpy as np
from rulesynthesis.metanet_attn import MetaNetRNN, AttnDecoderRNN, DecoderRNN, describe_model, WrapperEncoderRNN
from rulesynthesis.masked_cross_entropy import *
from rulesynthesis.interpret_grammar import Grammar, Rule
import dill
import rulesynthesis.util
...
def get_episode_generator(episode_type):
    ...
    if episode_type in ['scan_random',
        ...
        generate_episode_train, generate_episode_test, input_lang, output_lang, _ = rulesynthesis.util.get_episode_generator(episode_type)
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(fn_out_model) or args.cont:
        ...
        # assert False
        samples_val = [ rulesynthesis.util.cuda_a_dict(s) for s in samples_val ]
        ...
        for episode in range(start_episode, num_episodes+1):
            ...
            sample = rulesynthesis.util.cuda_a_dict(sample)
        ...
        if args.human_miniscan:
            from rulesynthesis.miniscan_state import examples_train, examples_test
        ...
        test_samples = [ rulesynthesis.util.cuda_a_dict(s) for s in test_samples ]
