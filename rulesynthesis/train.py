...
def gen_samples(generate_episode_train, model):
    ...
    if curtime > batchtime['max']:
        batchtime['max'] = curtime

    # get new mean batchtime
    batchtime['mean'] = (batchtime['mean'] * batchtime['count'] + curtime) / \
                        (batchtime['count'] + 1)
...
def get_policy_loss(samples, model, eval_only=False):
    ...
    target_batches = torch.transpose(torch.cat([sample['g_padded'] for sample in samples], 0), 0,
...
def train_batched_step(batch_of_samples: List[Dict], model, eval_only=False):
    loss = get_policy_loss(batch_of_samples, model, eval_only=eval_only)
