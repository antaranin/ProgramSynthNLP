...
from rulesynthesis.batched_synth_net import BatchedDoubleAttnDecoderRNN, BatchedRuleSynthEncoderRNN
import torch
import numpy as np
from rulesynthesis.agent import Example, State
from rulesynthesis.agent import ParseError
import time
import copy
...
def sample_rules_batched(states, model, max_length=20, nosearch=False):
    ...
    samples = model.re_pad_batch(samples,
                                 eval_mode=True)  # TODO, i think this will crash bc no grammar
    ...
    decoder_input = torch.tensor(
        [model.prog_lang.symbol2index[SOS_token]] * batch_size)  # nq length tensor
    ...
    for t in range(max_length):
        ...
        if nosearch:
            ...
            dist = torch.distributions.categorical.Categorical(
                logits=decoder_output)  # should be batch_size x n_outputs
...
def batched_test_with_sampling(sample, model, examples=None, query_examples=None, max_len=10,
                               timeout=10, verbose=False,
                               min_len=0, batch_size=64, max_rule_size=40,
                               use_query_for_construction=True, nosearch=False,
                               partial_credit=False, seperate_query=False):
    if nosearch:
                ...
                if not new_state.examples and i + 1 >= min_len:
                    ...
                    try:
                        ...
                        if use_query_for_construction:
                            ...
                            if not all(
                                    c in model.output_lang.symbols for ex in testout2.examples for c
                                    in ex.current):
                                if verbose:
                                    print(
                                        "skipping this one bc didn't parse")  # is this okay?????
                                continue
                            else:
                                r = 0
                    ...
                    if not testout.examples:
                        ...
                        if verbose:
                            print("solved given examples but failed query")
                        hit = False
                        solution = new_state
                        stats['end_time'] = time.time()
