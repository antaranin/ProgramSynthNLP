...
import numpy as np
import rulesynthesis.generate_episode as ge
...
def build_sample(x_support, y_support, x_query, y_query,
    ...
    sample['xs_padded'], sample['xs_lengths'] = build_padded_var(x_support, input_lang,
                                                                 integerIO=integerIO)  # (ns x max_length)
    sample['ys_padded'], sample['ys_lengths'] = build_padded_var(y_support, output_lang,
                                                                 integerIO=integerIO)  # (ns x max_length)
    sample['xq_padded'], sample['xq_lengths'] = build_padded_var(x_query, input_lang,
                                                                 integerIO=integerIO)  # (nq x max_length)
    sample['yq_padded'], sample['yq_lengths'] = build_padded_var(y_query, output_lang,
                                                                 integerIO=integerIO)  # (nq x max_length)
...
def generate_rules_episode(nsupport,
    ...
    return build_sample(x_support, y_support, x_query, y_query, input_lang, output_lang, prog_lang,
                        myhash, grammar=G)
...
def generate_scan_episode(nsupport,
    ...
    while True:
        G = ge.generate_scan_rules(nprims, nurules, nxrules, input_symbols, output_symbols,
                                   u_type=u_type)
        myhash = make_hashable(G)
        try:
            D = ge.sample_data(ntotal, G, maxlen_input=maxlen, maxlen_output=maxlen,
                               out_lang=output_lang, input_lang=input_lang)
        except DataSamplingError:
            # print("hit a bad datum!")
            continue
        np.random.shuffle(D)
    ...
    if model_in_lang and model_out_lang and model_prog_lang:
        return build_sample(x_support, y_support, x_query, y_query, model_in_lang, model_out_lang,
                            model_prog_lang, myhash, grammar=G)
    else:
        return build_sample(x_support, y_support, x_query, y_query, input_lang, output_lang,
                            prog_lang, myhash, grammar=G)


def get_episode_generator(episode_type, model_in_lang=None, model_out_lang=None,
                          model_prog_lang=None):
    ...
    if episode_type == "NLP":
        ...
        prog_lang = Lang(
            input_lang.symbols + output_lang.symbols + ['->', '\n', 'x1', 'u1', '[x1]', '[u1]',
                                                        'x2', '[x2]', 'u2', '[u2]'])
        ...
        def generate_episode_train(tabu_episodes):
            ...
            return generate_rules_episode(nsupport=nsupp, nquery=10, nprims=nprims, nrules=nrules,
                                          input_lang=input_lang, output_lang=output_lang,
                                          prog_lang=prog_lang, tabu_list=tabu_episodes)

        generate_episode_test = generate_episode_train
    # elif episode_type == 'nlp':
    #     input_lang = Lang(alphabet + ["LEFT", "RIGHT", "INS", "DEL"])
    #     output_lang = Lang()
    #     prog_lang = Lang(input_lang)
    #
        ...
        from rulesynthesis.number_generate_model import generate_lang_test_episode
        from rulesynthesis.number_word_interpret_grammar import RHS_DICT
        tokens = ['token' + format(i, '02d') for i in range(1, 52)]
        input_lang = Lang(tokens)
        output_lang = Lang([str(i) for i in range(10)])
        prog_symbols = ['1000000*', '10000*', '1000*', '100*', '10*', '[x1]*10', '[x1]*100',
                        '[x1]*1000', '[x1]*10000', '[x1]*1000000', '[x1]', '[u1]', '[y1]', 'x1',
        ...
        from rulesynthesis.number_generate_model import generate_wordToNumber_episode
        from rulesynthesis.number_word_interpret_grammar import RHS_DICT
        tokens = ['token' + format(i, '02d') for i in range(1, 52)]
        input_lang = Lang(tokens)
        output_lang = Lang([str(i) for i in range(10)])
        ...
        words = ['walk', 'look', 'run', 'jump', 'turn', 'left', 'right', 'opposite', 'around',
                 'twice', 'thrice', 'and', 'after'] + ['dax', 'blicket', 'lug', 'kiki']
        ...
        words = ['walk', 'look', 'run', 'jump', 'turn', 'left', 'right', 'opposite', 'around',
                 'twice', 'thrice', 'and', 'after'] + ['dax', 'blicket', 'lug', 'kiki']
        cmds = ['I_WALK', 'I_LOOK', 'I_RUN', 'I_JUMP', 'I_TURN_LEFT', 'I_TURN_RIGHT'] + ['RED',
                                                                                         'BLUE',
                                                                                         'GREEN']
        ...
        raise Exception(f"episode_type {episode_type} is not valid")
...
def generate_traditional_synth_scan_episode(nsupport, nquery, input_lang, output_lang, train_tuples,
                                            test_tuples, tabu_list=[]):
    ...
    return build_sample(x_support, y_support, x_query, y_query, input_lang, output_lang,
                        prog_lang=None, myhash='', grammar='')


def generate_batchsize_of_samples(sample_generator, batchsize=200):
    '''
    Takes a generation function and outputs lists of optimal size samples
    :param sample_generator: Function that generates samples
    :param batchsize: The size of the batch that will be generated and returned
    :return: A list of samples in the provided batch size
    '''
    remainder = []
    while True:
        previous_sample_remainder = remainder
        if len(previous_sample_remainder) > batchsize:
            yield previous_sample_remainder[:batchsize]
            remainder = previous_sample_remainder[batchsize:]
            continue

        sample = sample_generator()
        sample = previous_sample_remainder + sample
        ln = len(sample)

        if ln > batchsize:
            yield sample[:batchsize]
            remainder = sample[batchsize:]
            continue
        elif ln < batchsize:
            remainder = sample
            continue
            ...
            assert 0, "uh oh, not a good place"
...
class GenData:
    def __init__(self, fn, n_processes=4, max_size=200, batchsize=200):
        ...
        def consumer(Q):
            iterator = generate_batchsize_of_samples(fn, batchsize=batchsize)  # todo
            while True:
                try:
                    # get a new message
                    size = Q.qsize()
